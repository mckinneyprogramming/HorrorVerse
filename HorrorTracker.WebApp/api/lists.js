...
async function loadLists(connectionString, userId) {
  const rows = await queryRows(
    connectionString,
...
async function createList(connectionString, userId, name) {
...
async function updateList(connectionString, userId, body) {
  ...
  try {
    const rows = await queryRows(
      connectionString,
      visibility ? "UPDATE user_list SET name = $1, visibility = $2 WHERE id = $3 AND user_id = $4 RETURNING id" : "UPDATE user_list SET name = $1 WHERE id = $2 AND user_id = $3 RETURNING id",
...
async function deleteList(connectionString, userId, listId) {
...
async function addListItem(connectionString, userId, body) {
...
async function removeListItem(connectionString, userId, listId, itemId) {
...
async function addFranchise(connectionString, userId, body) {
  const listId = await requireOwnedList(connectionString, userId, body.listId ?? body.id);
...
async function removeFranchise(connectionString, userId, listId, franchiseId) {
  const ownedListId = await requireOwnedList(connectionString, userId, listId);
...
async function addSeriesMovies(connectionString, listId, seriesId) {
  try {
    await execute(
      connectionString,
      `INSERT INTO user_list_item (list_id, media_kind, media_id)
...
async function removeSeriesMovies(connectionString, listId, seriesId) {
  try {
    await execute(
      connectionString,
      `DELETE FROM user_list_item
...
async function requireOwnedList(connectionString, userId, listId) {
  const id = requireListId(listId);
...
function duplicateListError(error) {
  const message = error instanceof Error ? error.message : "";
...
var LibraryError = class extends HttpError {
};
